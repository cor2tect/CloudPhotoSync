...
namespace CameraControl
{
    class DownloadCommand : Command
    {
        ...
        public override bool Execute()
        {
            ...
            {
                Utility.g_fileExtension = Path.GetExtension(dirItemInfo.szFileName);
            ...
            Utility.g_ms.Position = 0;

            //saving into the memory stream
            using (FileStream file = new FileStream(dirItemInfo.szFileName, FileMode.Open, FileAccess.Read))
                file.CopyTo(Utility.g_ms);

            try { File.Delete(dirItemInfo.szFileName); } catch{ }
